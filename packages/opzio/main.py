...
def parseArguments():
    """Parse command line arguments and configs from ~/.opzio.cfg.
    ...
    parser.add_argument('--key', dest='key',
            help='your Opzio api key; uses api_key from '+
                '~/.opzio.cfg by default')
    ...
    if not args.logfile and os.environ.get('WAKATIME_HOME'):
        home = os.environ.get('WAKATIME_HOME')
        args.logfile = os.path.join(os.path.expanduser(home), '.opzio.log')
...
def get_user_agent(plugin):
    ...
    user_agent = u('opzio/{ver} ({platform}) Python{py_ver}').format(
        ver=u(__version__),
...
def send_heartbeat(project=None, branch=None, hostname=None, stats={}, key=None,
    ...
    if not api_url:
        api_url = 'http://api.opz.io/v1/logs'
    if not timeout:
        timeout = 60
