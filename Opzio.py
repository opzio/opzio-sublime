...
API_CLIENT = os.path.join(PLUGIN_DIR, 'packages', 'opzio', 'cli.py')
SETTINGS_FILE = 'Opzio.sublime-settings'
SETTINGS = {}
LAST_HEARTBEAT = {
    'time': 0,
    'file': None,
...
try:
    from opzio.base import parseConfigFile
except ImportError:
    pass
...
def log(lvl, message, *args, **kwargs):
    try:
        ...
        try:
            print('[Opzio] [{lvl}] {msg}'.format(lvl=lvl, msg=msg))
        except UnicodeDecodeError:
            print(u('[Opzio] [{lvl}] {msg}').format(lvl=lvl, msg=u(msg)))
    except RuntimeError:
...
def resources_folder():
    if platform.system() == 'Windows':
        return os.path.join(os.getenv('APPDATA'), 'Opzio')
    else:
        return os.path.join(os.path.expanduser('~'), '.opzio')
...
def prompt_api_key():
    ...
    if SETTINGS.get('api_key'):
        ...
        if window:
            window.show_input_panel('[Opz.io] Enter your opz.io api key:', default_key, got_key, None, None)
            return True
        else:
...
class SendHeartbeatsThread(threading.Thread):
    ...
    def send_heartbeats(self):
        if python_binary():
            ...
            ua = 'sublime/%d sublime-opzio/%s' % (ST_VERSION, __version__)
            cmd = [
                python_binary(),
                API_CLIENT,
            ...
            try:
                ...
                if retcode:
                    log(DEBUG if retcode == 102 else ERROR, 'opzio-core exited with status: {0}'.format(retcode))
                if output:
                    log(ERROR, u('opzio-core output: {0}').format(output))
            except:
...
def plugin_loaded():
    ...
    log(INFO, 'Initializing Opz.io plugin v%s' % __version__)
    ...
    if not python_binary():
        ...
        if platform.system() == 'Windows':
            ...
            sublime.error_message("Unable to find Python binary!\Opz.io needs Python to work correctly.\n\nGo to https://www.python.org/downloads")
            return
...
class WakatimeDashboardCommand(sublime_plugin.ApplicationCommand):
    ...
    def run(self):
        webbrowser.open_new_tab('https://opz.io/dashboard')
